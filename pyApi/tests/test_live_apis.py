...
def setUpModule():
    # Standard unittest pattern — raising SkipTest from setUpModule plays
    # nicely with `unittest discover` (no sys.exit during import). The
    # original script used sys.exit which breaks discovery on Python 3.14.
    if not RUN_LIVE:
        raise unittest.SkipTest(
            "Live API tests are skipped by default. "
            "Run with: RUN_LIVE=1 python3 -m unittest tests.test_live_apis"
        )
