...
| `tests/test_minority.py`   | 👥 Crowd engine (`minority.py`): Challet–Zhang bots refereed by the real market — rigged strategy tables for deterministic rounds, warm-up pre-training, virtual-score updates, leaderboard. No DB. |
...
the test count after the change (`+8 tests → 676 green`). When you
