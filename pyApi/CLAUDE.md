...
- Streamlit dashboard with 6 analytical tabs (Score, Analysis, Backtest, Alerts, Briefing, Collect) plus 4 sidebar pages: ⚙️ Admin (edit watchlist, trigger collects, inspect DB), 🎮 Game (paper-trading portfolio), ⏪ Replay (time-travel betting game on historical bars — single/portfolio/minority modes, session-only, nothing stored; engines in `stock_toolkit/replay.py` and `stock_toolkit/minority.py`), and ❓ Help (in-app orientation for new users)
