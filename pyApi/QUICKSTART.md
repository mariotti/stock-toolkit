...
| **🎮 Game** *(sidebar)* | Paper-trading portfolios (multiple "strategies" in parallel): start with virtual cash, buy fractional shares of any symbol with collected data at the latest close (+0.1% slippage), check back tomorrow / next week / next month to see how it played out. Switch between strategies in the top-of-page selector; create new ones via the "+ New strategy" expander; per-strategy rename / reset / archive / delete in Settings. State lives in `portfolio.db` |
